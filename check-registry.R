...
if (length (index) > 0) {
    ...
    hrefs <- paste0 (
        ...
        pj_mismatch$iss_no,
        "  name '", pj_mismatch$package_current, "' was '",
        pj_mismatch$package_onboarded, "' in review thread"
    )
    iss_msg <- paste0 (
        "The following software review issue",
        ifelse (length (index) > 1, "s", ""),
        " have package names which do not match those in the issue templates:"
    )
    iss_end_comment <- paste0 (
        "\nTo fix, update the values in the review threads to reflect ",
        "current package names. The review thread values come from the ",
        "YAML template where present, or else the 'Package' value in ",
        "the full DESCRIPTION text. Update one or both of these to fix. ",
        "Issue titles may be left in whatever form they currently are."
    )
    iss_msg <-
